...
def get_convos():
    ...
    with open(file_path, 'rb') as f:
        for line in f.readlines():
            ...
            if len(parts) == 4:
                convo = []
                for line in parts[3][1:-2].split(', '.encode('utf8')):
...
def basic_tokenizer(line, normalize_digits=True):
    ...
    for fragment in line.strip().lower().split():
        for token in _WORD_SPLIT.split(fragment):
            if not token:
                continue
            if normalize_digits:
#                token = re.sub(_DIGIT_RE, b'#', token)
                token = _DIGIT_RE.sub(b'#', token)
            words.append(token)
    return words
        ...
        for word in sorted_vocab:
            if vocab[word] < config.THRESHOLD:
                with open('config.py', 'ab') as cf:
                    if filename[-3:] == 'enc':
                        cf.write(b'ENC_VOCAB = ' + str(index).encode('utf8') + b'\n')
                    else:
                        cf.write(b'DEC_VOCAB = ' + str(index).encode('utf8') + b'\n')
                break
            f.write(word + b'\n')
            index += 1
...
def process_data():
    ...
    print('Preparing vocab for enc')
    build_vocab('train.enc')
    print('Preparing vocab for dec')
    build_vocab('train.dec')
    print('Preparing token2id')
    token2id('train', 'enc')
    token2id('train', 'dec')
    token2id('test', 'enc')
    token2id('test', 'dec')
